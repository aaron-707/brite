...
## 3. Gate (Commits `acebfc0` and `82d9ea0`)
> [!IMPORTANT]
> **NEEDS HUMAN REVIEW**
...
- **Second Attempt**: Replaced TF-IDF with a targeted term-overlap check. For each citation (e.g., "§5.4"), the gate extracts up to 6 non-stopword tokens (minimum length 2 characters) immediately preceding the citation in the source sentence. It then checks if any of these tokens (or their 5-character prefixes for stemming) appear in the resolved target clause(s). If overlap is zero, it flags a `FLAG_CONFLICT` ("dead reference").

### Known Limitations
- The term-overlap check does not perform term-specificity weighting. Common terms like "period", "under", "limit", or "part" can trigger false-positive matches if they happen to appear in the target text. While this prevents false dead-reference flags on legitimate clauses, a more robust NLP approach or dependency parser might be required for complete safety.
